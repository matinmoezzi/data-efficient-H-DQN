...
class DQN(Base_Agent):
    ...
    def get_q_network_local_action_values(self, state):
        """Plots the q network local action values for the given state"""
        state = torch.from_numpy(state).float().unsqueeze(0).to(self.device)
    ...
    def hinton_diagram(self, matrix, max_weight=None, ax=None):
    ...
    def plot_hinton_for_all_state_action_pairs(self):
        """Plots the hinton diagram for all state action pairs"""
        state_action_pairs = []
        for state in range(6):
            s = np.array([state])
            action_values = self.get_q_network_local_action_values(s)
            print(action_values)
            state_action_pairs.append(action_values)
        self.hinton_diagram(np.array(state_action_pairs).T)
