...
class h_DQN_ER(Base_Agent):
    ...
    def __init__(self, config):
        ...
        self.controller.q_network_target = self.create_NN(input_dim=self.state_size*2, output_dim=self.action_size,
                                                         key_to_use="CONTROLLER")
        ...
        while not self.episode_over:
            ...
            while not (self.episode_over or self.subgoal_achieved):
                ...
                self.save_experience(memory=self.meta_controller.memory,
                                     experience=(self.meta_controller_state, self.state[0], self.cumulative_meta_controller_reward,
                                                 self.state[0], self.episode_over))
